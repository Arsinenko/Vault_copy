...
func CreateApp(Name string, Description string, OwnerID int32, metadata pgtype.JSONB) int {
	logHash := hex.EncodeToString(cryptoOperation.SHA256(append([]byte(Name+Description+string(OwnerID)), metadata.Bytes...)))
...
func API_AppChangeName(UserID int32, AppID int32, name string) int {
	logHash := hex.EncodeToString(cryptoOperation.SHA256([]byte(string(UserID) + string(AppID) + name)))

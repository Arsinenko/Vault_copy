...
func get_usr(phone_mail string) (models.User, error) {
	...
	if res.Error != nil { return user, res.Error; }
	return user, nil;
}
...
func CreateUser(phone_mail string, password string, full_name string) int {
	...
	if err == nil { return http.StatusInternalServerError } // [http]::Conflict - User with that phone or mail already exists
	if act_usr.Email == phone_mail || act_usr.PhoneNumber == phone_mail { return http.StatusConflict; }
